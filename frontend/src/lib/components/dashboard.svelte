<script lang="ts">
	...
	import { turmas as turmasApi, chamadas as chamadasApi, alunos as alunosApi } from '$lib/api';
	...
	let totalAlunosGeral = $state(0);
	let todasChamadas = $state<ChamadaItem[]>([]);

	onMount(async () => {
		const [ts, todosAlunos] = await Promise.all([
			turmasApi.listar(),
			alunosApi.listar() // ← busca alunos direto
		]);

		codTurmas = ts.map((t) => t.cod);
		totalAlunosGeral = todosAlunos.length; // ← conta alunos reais

		const promises = ts.map((t) => chamadasApi.listarPorTurma(t.cod));
		const resultados = await Promise.all(promises);
		todasChamadas = resultados.flat();
	});

	let frequenciaMediaGeral = $derived(() => {
		if (!todasChamadas.length) return 0;
		const pct = (todasChamadas.filter((c) => c.presente).length / todasChamadas.length) * 100;
...
<div class="flex flex-col gap-6">
	<!-- Estatísticas Gerais -->
	<div>
		<h2 class="mb-3 text-xs font-semibold tracking-widest text-gray-400 uppercase">Visão Geral</h2>
		<div class="flex flex-row justify-between gap-4">
			<div
				class="flex flex-1 flex-row items-center justify-between rounded-2xl bg-white p-4 shadow-sm"
			>
			...
			>
				<div class="flex flex-col gap-1">
					...
					<p class="text-3xl font-bold text-gray-800">{totalAlunosGeral}</p>
				</div>
			...
			>
				<div class="flex flex-col gap-1">
					...
					<p class="text-3xl font-bold text-gray-800">
						{frequenciaMediaGeral()}<span class="text-lg text-gray-400">%</span>
					</p>
				</div>
				<div class="flex h-12 w-12 items-center justify-center rounded-full bg-red-100">
					...
					>
						<path
							d="M280-280h160v-280H280v280Zm240 0h160v-560H520v560ZM200-120q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h560q33 0 56.5 23.5T840-760v560q0 33-23.5 56.5T760-120H200Z"
						/>
					</svg>
				</div>
			</div>
	...
	<div>
		<div class="mb-3 flex flex-row items-center justify-between">
			<h2 class="text-xs font-semibold tracking-widest text-gray-400 uppercase">
				Dashboard da Turma
			</h2>
			<select
		...
		{#if turmaSelecionada}
			...
			<div class="mb-4 flex flex-row gap-4">
				<div class="flex flex-1 flex-col gap-0.5 rounded-2xl bg-white p-4 shadow-sm">
				...
				<div class="flex flex-1 flex-col gap-0.5 rounded-2xl bg-white p-4 shadow-sm">
					...
					<p class="text-2xl font-bold text-gray-800">
						{totalAlunos ? Math.round((presentes / totalAlunos) * 100) : 0}<span
			...
			<!-- Gráficos -->
			<div class="mb-4 flex flex-row gap-4">
				<div class="flex flex-1 flex-col rounded-2xl bg-white p-4 shadow-sm">
			...
			<div class="flex flex-col rounded-2xl bg-white p-4 shadow-sm">
				<p class="mb-3 text-xs font-semibold tracking-widest text-gray-400 uppercase">
		...
		{:else}
			<div
				class="flex flex-col items-center justify-center gap-2 rounded-2xl bg-white p-12 text-center shadow-sm"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					height="40px"
